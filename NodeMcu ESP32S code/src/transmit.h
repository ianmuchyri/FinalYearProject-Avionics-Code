...
void sendTelemetryWiFi(Data dt)
{

  // publish whole message i json
  char mqttMessage[200];

  sprintf(mqttMessage, "{\"timestamp\":%lld,\"altitude\":%.3f,\"temperature\": %.3f,\"ax\":%.3f,\"ay\":%.3f,\"az\":%.3f,\"gx\":%.3f,\"gy\":%.3f,\"gz\":%.3f,\"filtered_s\":%.3f,\"filtered_v\":%.3f,\"filtered_a\":%.3f,\"state\":%d}", dt.timeStamp, dt.altitude,dt.temperature, dt.ax, dt.ay, dt.az, dt.gx, dt.gy, dt.gz, dt.filtered_s, dt.filtered_v, dt.filtered_a, dt.state);
  client.publish("esp32/message", mqttMessage);
}

void handleWiFi(Data dt)
{
  if (!client.connected())
  {
    reconnect();
